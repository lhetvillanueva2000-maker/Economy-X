...
Download `EconomyX_v2.8.mcaddon` and open it — Minecraft imports both packs
for you. Then, in your world settings, enable **both**:

- `EconomyX v2.8 [BP]` under Behavior Packs
- `EconomyX v2.8 [RP]` under Resource Packs
...
2. Delete `EconomyX_v2.8_RP/ui/server_form.json`. Nothing else reads it.
...
Everything worth balancing lives in `EconomyX_v2.8_BP/scripts/config.js`.
...
```
EconomyX_v2.8_BP/          behaviour pack
...
EconomyX_v2.8_RP/          resource pack
...
- **The UI skin is unverified in-game.** It restyles EconomyX's forms via
  ...
  `EconomyX_v2.8_RP/ui/server_form.json` reverts the look and changes nothing
  else.
