...
export function initPhones(injected) {
  ...
  /* ---- A phone and a book are not tools ----
   * Holding either and tapping a block would mine it, and in creative that is
   * instant — so sneak + use to open a screen would chew a hole in the world.
   * Neither item can break anything now, in any game mode.
   */
  safeSubscribe("phoneBookBreakGuard", () => {
    world.beforeEvents.playerBreakBlock.subscribe((event) => {
      try {
        ...
        if (id === BOOK_ID || (id && PHONE_ID_SET.has(id))) event.cancel = true;
      } catch {
        /* leave the break alone rather than risk cancelling something else */
      }
    });
  });
