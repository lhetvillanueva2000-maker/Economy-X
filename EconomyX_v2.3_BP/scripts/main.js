...
/* ------------------------------------------------------------
 *  Machine protection and the EX Pickaxe
 * ---------------------------------------------------------- */
...
/** Sneak + tap with the EX Pickaxe: dismantle a machine straight into the bag. */
function tryPickaxeHarvest(player, block) {
  try {
    if (!player.isSneaking) return false;
    const held = getHandItemRaw(player);
    if (!held || held.typeId !== "ex:ex_pickaxe") return false;
...
function routeMachine(player, block) {
  ...
  if (typeId === CONFIG.blocks.atm) p = handleAtmInteract(player, block);
    ...
    actionBar(player, "§eOnly the EX Pickaxe can dismantle this machine");
    holdActionBar(player, 30);
    return;
  }
...
safeSubscribe("playerBreakBlockGuard", () => {
  world.beforeEvents.playerBreakBlock.subscribe((event) => {
    try {
      ...
      // The EX Pickaxe is the ONLY thing that can dismantle a machine, in
      // creative and survival alike.
      if (heldId === "ex:ex_pickaxe") {
